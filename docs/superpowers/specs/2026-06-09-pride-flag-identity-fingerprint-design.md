# Pride-flag identity fingerprint — design
...
Show a small **pride-flag chip + two-word label** derived deterministically from
`seed + account`. The user's reference identity is vanity-mined to land on the **trans
flag**; any other identity lands on a *different* pride flag and shows a different word.
The flag is the instant joy/recognition signal; the word is a high-entropy backstop for
the rare case where a wrong identity collides on the same flag.
...
Fixed, ordered list; index 0 is the trans flag (the vanity target):

`trans` (target), `rainbow`, `bi`, `pan`, `lesbian`, `nonbinary`, `ace`, `genderfluid`,
`agender`, `genderqueer`.

All render as clean horizontal colored stripes (see brainstorm mockup
`.superpowers/brainstorm/.../flag-gallery.html` for exact hex values) and are mutually
...
equals `TRANS_INDEX` (~10 iterations on average, all fast SHA-256 over the already-derived
`fpBytes`). The resulting nonce is stored.
...
- **Seed or account legitimately changes** → the flag is no longer trans (it is now a
  genuinely different identity). The user re-taps "set as my flag" if the new identity
