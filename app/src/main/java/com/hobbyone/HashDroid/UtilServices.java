...
class UtilServices {
    ...
    public static String[] get_words(BigInteger int_data, int count) throws Exception {
        String[] res = new String[count];
        int added = 0;
        while (int_data.signum() == 1) {
            BigInteger[] t = int_data.divideAndRemainder(BigInteger.valueOf(2048));
            int_data = t[0];
            int mod = t[1].intValue();
            res[added] = wordlist[mod];
            added++;
