...
class DgpEngineTest {
    ...
    fun subaccount_goldenVectors_matchPythonReference() {
        ...
        assertEquals("FirmCaseClumpMatterFaultClickApologyThumbHugeFaultExciteWasteTiredDishSettleSayBlessGapRemoveLatinNoodleUndoOrbitResemble", engine.deriveSubaccountSeed("correct horse battery staple", "", "agent-bob"))
        assertEquals("AnswerAssaultWhaleUsedChucklePillFallPanicCoconutPartyWingLoopBlanketAuditAerobicObligeFeelFrozenFindGrowWiseProjectJunkNapkin", engine.deriveSubaccountSeed("correct horse battery staple", "alice@example.com", "agent-bob"))
        assertEquals("OuterBuddyBounceCinnamonBounceVacantVoyageLanguageVeryArtistHawkSketchForestCushionWoolFoamAbleCongressProsperNapkinUsualRugAssumeMatrix", engine.deriveSubaccountSeed("correct horse battery staple", "", "agent-alice"))
    }
}
